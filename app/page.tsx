...
import { ShoppingBag, ArrowRight, Sparkles, Compass, Box, Flame, ShieldCheck } from 'lucide-react';
...
export default function SplashScreen() {
  ...
  // คำนวณพิกัดเมาส์สำหรับ 3D Perspective Tilt
  ...
  const experimentalNodes = [
    { id: '3d-models', label: '3D Orbit', icon: Box, path: '/home?view=3d', color: 'from-cyan-500 to-blue-600' },
    { id: 'trending', label: 'Hot Deals', icon: Flame, path: '/home?view=hot', color: 'from-amber-500 to-rose-600' },
  ...
  return (
    ...
    >
      {/* 1. Dynamic Immersive Spotlight Layer */}
      <div
      ...
      <div className="absolute top-1/4 -left-20 w-80 h-80 bg-purple-600/30 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute bottom-1/4 -right-20 w-80 h-80 bg-indigo-600/30 rounded-full blur-3xl pointer-events-none" />

      {/* Header Bar */}
      <div className="w-full flex justify-between items-center pt-2 z-20 max-w-md">
        <span className="flex items-center gap-1.5 text-[11px] font-medium text-indigo-300 bg-slate-900/80 border border-indigo-500/30 px-3 py-1 rounded-full backdrop-blur-md shadow-lg">
      ...
      {/* 2. Main 3D Interactive Card (Perspective Elements) */}
      <div
      ...
      >
        <div className="flex flex-col items-center text-center p-8 rounded-3xl bg-slate-900/70 border border-white/15 backdrop-blur-2xl shadow-[0_20px_50px_rgba(0,0,0,0.8)] max-w-sm relative overflow-hidden">
          
          {/* Shopping Bag Icon with Floating Depth Effect */}
          <div 
            className="relative mb-6 transition-transform duration-300"
            style={{ transform: 'translateZ(40px)' }}
          >
            <div className="absolute inset-0 rounded-3xl bg-indigo-500 blur-xl opacity-60 animate-pulse" />
          ...
          <p 
            className="mt-3 text-xs leading-relaxed max-w-xs text-slate-300 font-normal"
            style={{ transform: 'translateZ(20px)' }}
          >
            ศูนย์รวมตลาดนัดออนไลน์ชาววิทยาลัย สัมผัสประสบการณ์ช้อปปิ้งมิติใหม่ในรั้วเดียวกัน
          </p>

          {/* 3. Experimental Immersive Navigation Nodes (ปุ่มทางลัดท่องมิติ 3D) */}
          <div 
            className="mt-6 w-full pt-4 border-t border-slate-800/80"
            style={{ transform: 'translateZ(35px)' }}
          >
            <p className="text-[10px] text-indigo-300/80 uppercase tracking-widest font-semibold mb-3">
                ...
                return (
                  <button
                    ...
                    className={`flex flex-col items-center gap-1.5 p-2.5 rounded-2xl border transition-all duration-300 ${
                      isSelected
                        ? 'bg-indigo-600/40 border-indigo-400 scale-105 shadow-[0_0_15px_rgba(99,102,241,0.5)]'
                        : 'bg-slate-800/50 border-slate-700/60 hover:border-slate-500 hover:bg-slate-800'
                    }`}
                  >
      ...
      {/* 4. Action Area & Immersive Progress Bar */}
      <div className="w-full max-w-xs space-y-4 z-20 mb-6">
        <button
